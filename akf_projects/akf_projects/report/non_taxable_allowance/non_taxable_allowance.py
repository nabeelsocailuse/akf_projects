...
def execute(filters=None):
	...
	# return columns, data
		...
		for e in earning_types:
			...
			total_allownce += flt(amount) 
			 # Store calculated total allowance
		row["total_allownce"] = total_allownce 
		...
		data.append(row)  # Append modified row to data
...
def get_earning_and_deduction_types(salary_slips):
	...
	for salary_compoent in get_salary_components(salary_slips):
		print("Salary Components:",salary_compoent)
  
		# get_total_allowance(salary_slips)
...
def get_columns(earning_types, ded_types):
	...
	columns.extend(
		[
			...
			# },
   			{
				"label": _("Total Allownce"),
				"fieldname": "total_allownce",
				"fieldtype": "Currency",
				"options": "currency",
				"width": 120,
			},
...
def get_salary_components(salary_slips):
	return (
		...
		.where((salary_detail.amount != 0) & (salary_detail.parent.isin([d.name for d in salary_slips])) & (salary_detail.is_tax_applicable == 0))
...
def get_salary_slip_details(salary_slips, currency, company_currency, component_type):
	...
	result = (
		...
		.where((salary_detail.parent.isin(salary_slips)) & (salary_detail.parentfield == component_type)&  (salary_detail.is_tax_applicable == 0))
		.select(
			salary_detail.parent,
			salary_detail.salary_component,
			salary_detail.amount,
			salary_slip.exchange_rate,
   			salary_detail.is_tax_applicable
		)
	).run(as_dict=1)
