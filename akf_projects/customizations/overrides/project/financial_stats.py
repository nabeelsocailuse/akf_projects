...
def get_donors(filters):
	def get_conditions():
		conditions = " and cost_center = %(cost_center)s " if(filters.get('cost_center')) else ""
		conditions += " and pay_service_area = %(service_area)s " if(filters.get('service_area')) else ""
		conditions += " and pay_subservice_area = %(subservice_area)s " if(filters.get('subservice_area')) else ""
		conditions += " and pay_product = %(product)s " if(filters.get('product')) else ""
		conditions += " and project = %(project)s " if(filters.get('project')) else ""
		return conditions
	
	return frappe.db.sql(f""" select donor_id, donor_name, equity_account
			from `tabPayment Detail`
			where docstatus=1
			group by donor_id, equity_account
			{get_conditions()}
			""", filters, as_dict=1)
	
@frappe.whitelist()
def get_donor_transactions(filters=None):
	if(type(filters) == str): filters = ast.literal_eval(filters)
	# company_currency = erpnext.get_company_currency(filters.get("company"))
	donors_balance = []
	for d in get_donors(filters):
		filters.update({
      		"donor": d.donor_id,
			"account": d.equity_account,
			# "": company_currency
        })
		donation = get_donation(filters)
		fund = get_funds_transfer(filters)
		purchase = get_purchasing(filters)
		
		balance = donation["donation"] + donation["received_pledge"] + fund["received_funds"] - fund["transfered_funds"] - purchase["purchased_amount"]
		
		if(balance>0.0):
			donors_balance.append({
       			'donor': d.donor_id, 
